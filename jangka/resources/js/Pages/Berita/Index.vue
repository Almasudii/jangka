...
const beritaItems = computed(() => props.berita?.data || [])
...
function submitForm() {
  if (isEditing.value && selectedBerita.value) {
    form
      ...
      .post(route('berita.update', selectedBerita.value.id), {
        forceFormData: true,
        preserveScroll: true,
        onSuccess: () => {
          closeFormModal()
...
function deleteBerita() {
  ...
  router.delete(route('berita.destroy', selectedBerita.value.id), {
    preserveScroll: true,
    onSuccess: () => {
      closeDeleteModal()
    },
...
function shortText(text, maxLength = 135) {
  if (!text) return '-'
  ...
  <DashboardLayout>
    <div class="min-h-screen bg-slate-50 text-slate-900">
      <div class="mx-auto max-w-7xl px-5 py-8 sm:px-8 lg:px-10">
          ...
          <div class="relative flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p class="mt-4 max-w-2xl text-sm leading-6 text-blue-100 sm:text-base">
                Update informasi terkini seputar kegiatan, pengumuman, dan layanan
                Pemerintah Desa.
              </p>
            </div>
        ...
        <section class="mt-8">
          ...
          >
            ...
            <p class="mx-auto mt-2 max-w-md text-sm leading-6 text-slate-500">
              Data berita belum tersedia untuk ditampilkan. Admin dapat menambahkan
              berita baru melalui tombol tambah berita.
            </p>
          </div>
            ...
            >
              <div class="relative h-56 w-full overflow-hidden bg-slate-100">
                ...
                <div class="absolute bottom-4 right-4">
                  <span
                    class="inline-flex items-center gap-2 rounded-full bg-slate-950/80 px-3 py-2 text-xs font-bold text-white shadow-lg backdrop-blur transition group-hover:bg-blue-700"
                  >
              ...
              <div class="p-6">
                ...
                <h2 class="line-clamp-2 text-xl font-black leading-snug text-slate-950 transition group-hover:text-blue-700">
                  {{ item.judul }}
                </h2>

                <p class="mt-3 line-clamp-3 text-sm leading-6 text-slate-600">
                  {{ shortText(item.ringkasan || item.isi) }}
                </p>
                ...
                <div class="mt-6 flex items-center justify-between border-t border-slate-100 pt-5">
                  ...
                  <button
                    type="button"
                    class="inline-flex items-center gap-2 text-sm font-bold text-blue-700 transition hover:text-blue-900"
                  >
                    Selengkapnya
                    <Eye class="h-4 w-4" />
                  </button>
          ...
          <div
            v-if="props.berita?.links?.length > 3"
            class="mt-8 flex flex-wrap items-center justify-center gap-2"
          >
            <Link
              v-for="link in props.berita.links"
              :key="link.label"
              :href="link.url || '#'"
              v-html="link.label"
        ...
        <div
          v-if="showDetailModal && selectedBerita"
        ...
        >
          <div class="w-full max-w-4xl overflow-hidden rounded-[2rem] bg-white shadow-2xl">
            <div class="max-h-[90vh] overflow-y-auto">
              <div class="relative h-80 w-full overflow-hidden bg-slate-100">
                <img
                  v-if="thumbnailUrl(selectedBerita.thumbnail)"
                  :src="thumbnailUrl(selectedBerita.thumbnail)"
                ...
                >
                  <div class="text-center">
                    <div class="mx-auto flex h-20 w-20 items-center justify-center rounded-3xl bg-amber-100 text-amber-700 ring-1 ring-amber-200">
                      <ImageOff class="h-10 w-10" />
                    </div>

                    <p class="mt-4 text-base font-black uppercase tracking-wide text-amber-700">
                      Thumbnail Tidak Ada
                    </p>
                ...
                <button
                  ...
                  class="absolute right-5 top-5 flex h-11 w-11 items-center justify-center rounded-full bg-white text-slate-700 shadow-lg transition hover:bg-slate-100"
                >
                  <X class="h-5 w-5" />
                </button>
              </div>
              ...
              <div class="p-7 sm:p-8">
                <div class="mb-5 flex flex-wrap items-center gap-3 text-sm text-slate-500">
                  <span class="inline-flex items-center gap-1.5">
                    <CalendarDays class="h-4 w-4" />
                    {{ formatDate(selectedBerita.published_at || selectedBerita.created_at) }}
                  </span>
                  ...
                  <span class="inline-flex items-center gap-1.5">
                    <UserRound class="h-4 w-4" />
                    {{ selectedBerita.penulis?.name || 'Admin' }}
                  </span>
                  ...
                  >
                    {{ statusText(selectedBerita.status) }}
                  </span>
                </div>
                ...
                <div
                  v-if="isAdmin"
                  class="mt-7 flex flex-col gap-3 border-t border-slate-100 pt-6 sm:flex-row sm:justify-end"
                >
                  <button
                    type="button"
                    @click="openEditModal(selectedBerita)"
                    class="inline-flex items-center justify-center gap-2 rounded-xl bg-amber-500 px-5 py-3 text-sm font-bold text-white shadow-lg shadow-amber-500/20 transition hover:bg-amber-600"
                  >
                    <Edit3 class="h-4 w-4" />
                    Edit Berita
                  </button>
                  ...
                  <button
                    type="button"
                    @click="openDeleteModal(selectedBerita)"
            ...
            <div class="max-h-[90vh] overflow-y-auto">
              <div class="flex items-start justify-between border-b border-slate-100 bg-slate-50 p-7">
                <div>
                  ...
                  <p class="mt-1 text-sm text-slate-500">
                    Isi data berita yang akan ditampilkan di halaman berita desa.
                  </p>
                </div>
              ...
              >
                ...
                <div class="flex flex-col gap-3 border-t border-slate-100 pt-5 sm:flex-row sm:justify-end">
                  ...
                  >
                    <Save class="h-4 w-4" />
                    {{ form.processing ? 'Menyimpan...' : isEditing ? 'Update Berita' : 'Simpan Berita' }}
                  </button>
                </div>
              </form>
            </div>
        ...
        <div
          v-if="showDeleteModal && selectedBerita"
          class="fixed inset-0 z-[999] flex items-center justify-center bg-slate-950/60 px-4 backdrop-blur-sm"
        >
